...
class Particle extends Coloring{
    ...
    constructor(position, spawnBoundary = null) {
        
      super();
      
      this.boundary = spawnBoundary
    ...
    */
    ...
    /**
     * Checks for collision with left boundary
     */
    get collided_with_left() {
        const bounds = this.getBoundaryEdges();
        return this.position.x - this.radius <= bounds.left;
    }

    /**
     * Checks for collision with right boundary
     */
    get collided_with_right() {
        const bounds = this.getBoundaryEdges();
        return this.position.x + this.radius >= bounds.right;
    }

    /**
     * Checks for collision with top boundary
     */
    get collided_with_top() {
        const bounds = this.getBoundaryEdges();
        return this.position.y - this.radius <= bounds.top;
    }

    /**
     * Checks for collision with bottom boundary
     */
    get collided_with_bottom() {
        const bounds = this.getBoundaryEdges();
        return this.position.y + this.radius >= bounds.bottom;
    }
    ...
    /**
     * Gets the edges of the collision boundary
     * @returns {Object} Boundary edges to use for collision detection
     */
    getBoundaryEdges() {
        if (this.boundary) {
            return this.boundary.getBoundaryEdges();
        }
        // Default to canvas bounds if no boundary set
        return {
            left: 0,
            right: width,
            top: 0,
    ...
    /**
     * Determines valid initial position for particle spawn.
     * Respects boundary constraints if they exist.
     * 
     * @param {p5.Vector|null} position - Preferred spawn position or null
     * @returns {p5.Vector} Valid spawn position
     */
    getInitialPosition(position = null){
        // if no boundary is defined, use the entire canvas
        if (!this.boundary){
            return createVector(random(width), random(height))
        }

        // if a valid position (within the boundary) is provided, use that
        if (position && this.boundary.contains(position)){
            return position;
        }

        // return a random position within the boundary
        return this.boundary.getRandomPosition();
    }

    /**
     * Updates the spawn boundary for the particle.
     * Optionally repositions particle if it's outside new boundary.
     * 
     * @param {SpawnBoundary} boundary - New spawn boundary to use
     */
    setSpawnBoundary(boundary) {
        this.boundary = boundary;
        if (boundary && !boundary.contains(this.position)) {
            this.position = boundary.getRandomPosition();
        }
    }
    ...
    // Original update function that doesn't account for boundary
    // update() {
    ...
    // }
    /**
     * Updates particle position and handles boundary interactions.
     * Different boundary types have different containment behaviors:
     * - Circle boundaries cause bouncing
     * - Other boundaries wrap around edges
     */
    update() {
        // Update physics
        this.velocity.add(this.acceleration);
        ...
        // Handle boundary containment
        if (this.boundary) {
            const bounds = this.getBoundaryEdges();
            
            if (this.boundary.type === 'circle') {
                // Circular boundaries cause bouncing
                ...
                // Wrap position within boundary limits
                this.position.x = ((this.position.x - bounds.left + w) % w) + bounds.left;
                this.position.y = ((this.position.y - bounds.top + h) % h) + bounds.top;
            }
        } else {
            // Default canvas wrapping behavior
    ...
    /**
     * Handles bouncing behavior when particle collides with boundaries.
     * Reverses velocity and adjusts position to prevent sticking to boundaries.
     */
    bounce() {
        const bounds = this.getBoundaryEdges();
        ...
        if (this.sideCollision) {
            ...
            if (this.collided_with_left) {
                this.position.x = bounds.left + this.radius;
            } else if (this.collided_with_right) {
                this.position.x = bounds.right - this.radius;
            }
        }
        
        if (this.topBottomCollision) {
            this.velocity.y *= -1;
            // Ensure particle stays within bounds
            if (this.collided_with_top) {
                this.position.y = bounds.top + this.radius;
            } else if (this.collided_with_bottom) {
    ...
    move_towards(vector){
        ...
        // Normalizes the distance vector (distance between the ball and the mouse).
        // This controls how fast the distance between the ball and mouse is reduced by.
        // If the original magnitude is used, the ball will basically teleport to the mouse.
        // force.limit((50/magnitude));
        force.limit(magnitude / 1500);
        // force.limit(1);
